...
<?php if (isset($_POST['btn'])) {
    ...
    if (
        ...
        if ($FileSize <= 1000000) {
            ...
            if ($res) {
                echo "<script>alert('Data Inserted Successfully!!');window.location.href = 'ImageWork.php'</script>";
                move_uploaded_file($FileTempLoc, $folder);
            } else {
                echo "<script>alert('Data Insertion Failed!!');window.location.href = 'ImageWork.php'</script>";
            }
        } else {
            echo "<script>alert('Image should be less than 1MB!!');window.location.href = 'ImageWork.php'</script>";
        }
    } else {
        echo "<script>alert('Image Formate not supported!!');window.location.href = 'ImageWork.php'</script>";
    }
} ?>
